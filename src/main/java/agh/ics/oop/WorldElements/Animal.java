...
public class Animal implements IMapElement {
    ...
    public int energy;
    public ArrayList<Integer> genotype = new ArrayList<>(); // przydałaby się osobna klasa + po co Pan ma MoveDirection, skoro tu Pan trzyma Integery?
    ...
    public Vector2d getPosition() {
        return position;    // zwraca Pan na zewnątrz obiekt modyfikowalny
    }
    ...
    public void addEnergy(int energy) {
        this.energy = Math.min(this.energy + energy, this.map.getMaxAnimalEnergy());
    }

    public ArrayList<Integer> getGenotype() {
        return genotype;    // zwraca Pan na zewnątrz obiekt modyfikowalny
    }
    ...
    public void move() {
        ...
        switch (direction) {
            case FORWARD -> {
                ...
                if (!canMoveTo && map instanceof WrappedMap) {  // gdyby było więcej rodzajów mapy, to zwierzę musiałoby mieć świadomość każdego?
            ...
            case BACKWARDLEFT -> this.mapDirection = this.mapDirection.previous().previous().previous();    // przydałaby się pętla
    ...
    private MoveDirection chooseNewDirection() {
        ...
        switch (gene) {
            case 0 -> {
                return MoveDirection.FORWARD;   // to zdecydowanie warto skrócić
            }
            case 45 -> {
    ...
    public Label getLabel() {
        return new Label(Integer.toString(this.energy));
    }
    ...
    public void fillTheGenes() {
        ...
        while (this.genotype.size() < 32) {
            Random rand = new Random(); // nowy obiekt co obrót pętli, a wystarczyłby jeden na całą klasę
